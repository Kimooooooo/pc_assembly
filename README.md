...
사용자가 예산과 용도, 자유형식 참고사항을 입력하면 AI가 실시간으로 **가성비형 · 밸런스형 · 고성능형** 3종 PC 견적을 생성합니다.

- **RAG 파이프라인**: 실제 PC 부품 데이터를 ChromaDB 벡터 DB에 저장 후, 의미 기반 검색으로 부품을 추출
- **LangGraph 오케스트레이션**: 5개 노드로 구성된 상태 기반 파이프라인이 분석 → 검색 → 생성 → 검증 → 포맷 순으로 처리
- **OpenAI GPT-4o-mini**: JSON 구조화 출력으로 안정적인 견적 데이터 생성
...
| **벡터 DB** | ChromaDB 0.6.3 (HNSW index, Cosine similarity) |
...
flowchart TD
    ...
    subgraph LANGGRAPH["LangGraph 파이프라인 — StateGraph"]
        C["① analyze_request\nGPT-4o-mini\n예산 분배 · 우선순위 분석\n참고사항 키워드 추출"]
        C --> D
        D["② search_parts\nChromaDB 카테고리별 벡터 검색\nDense 임베딩으로 의미 기반 필터링"]
        D --> E
        E["③ generate_quotes\nGPT-4o-mini (JSON mode)\n가성비형 · 밸런스형 · 고성능형\n3종 견적 + 선정 이유 3줄 생성"]
        E --> F
        F["④ check_compatibility\nGPT-4o-mini\n소켓 · 전력 · 규격 호환성 검증\n비호환 시 대체 부품 교체"]
        F --> G
        G["⑤ filter_and_format\n서버 사이드 총 금액 재계산\n견적 ID 정렬 · reason 필드 검증"]
    end
    ...
    style LANGGRAPH fill:#1e293b,color:#94a3b8,stroke:#334155
    style DJANGO fill:#0f172a,color:#94a3b8,stroke:#1e3a5f
    style CHROMADB fill:#0f172a,color:#94a3b8,stroke:#1e3a5f
    style EMBEDDING fill:#0f172a,color:#94a3b8,stroke:#1e3a5f
    style FRONTEND fill:#0f172a,color:#94a3b8,stroke:#1e3a5f
    style C fill:#1d4ed8,color:#fff,stroke:#2563eb
    style E fill:#1d4ed8,color:#fff,stroke:#2563eb
    style F fill:#1d4ed8,color:#fff,stroke:#2563eb
```
...
| ② | `search_parts` | ChromaDB에서 각 카테고리별로 벡터 유사도 검색. 참고사항 쿼리는 Dense 임베딩으로 의미 기반 매칭 |
| ③ | `generate_quotes` | 검색된 부품 후보를 바탕으로 GPT-4o-mini가 3가지 유형의 견적을 JSON으로 구조화 생성 |
| ④ | `check_compatibility` | CPU 소켓-메인보드, 전력 소비-파워 용량, 규격(ATX/mATX) 등 호환성 검증 및 자동 교체 |
| ⑤ | `filter_and_format` | LLM이 반환한 총 금액 대신 서버에서 부품 가격을 직접 합산하여 신뢰도 보장 |
...
### 1. 3가지 유형의 견적 자동 생성
- **가성비형**: 예산 대비 최대 성능, 필수 부품 중심 구성
- **밸런스형**: 성능과 가격의 균형, 범용적 사용에 최적화
- **고성능형**: 예산 내 최고 성능, 프리미엄 부품 우선 선택

### 2. 서버 사이드 가격 계산
LLM이 반환하는 `total_price` 값의 신뢰성 문제를 해결하기 위해, 서버에서 각 부품 가격을 직접 합산하여 정확한 총액을 보장합니다.
...
### 3. AI 견적 선정 이유 제공
각 견적마다 LLM이 선정 이유를 3문장으로 요약하여 사용자에게 제공합니다.

### 4. 호환성 자동 검증
- CPU 소켓 ↔ 메인보드 소켓 일치 여부
- 전체 소비전력 ↔ 파워 용량 마진 확인
- 케이스 폼팩터 ↔ 메인보드 규격 적합성
...
pc_assembly/
├── pc_assembly/
│   ├── settings.py          # Django 설정 (SQLite/PostgreSQL 전환 지원)
│   └── urls.py
├── main/
│   ├── graph.py             # LangGraph 5-노드 파이프라인 핵심 로직
│   ├── views.py             # Django 뷰 — 요청 수신 및 에러 처리
...
```bash
cd pc_assembly
python main/vectordb.py
```

> `가공데이터/임베딩/` 폴더의 `.txt` 파일 12개를 읽어 ChromaDB를 구축합니다.
...
- `response_format={"type": "json_object"}` 옵션으로 구조화된 JSON 출력 보장
- 3종 견적 + 호환성 검증을 안정적으로 처리하는 충분한 컨텍스트 윈도우
...
### 서버 사이드 가격 계산
- LLM의 `total_price` 필드는 신뢰도가 낮아 서버에서 부품별 가격을 직접 합산
- `_parse_price_int()` 함수로 "1,360,680원" 형식의 문자열도 안전하게 파싱
