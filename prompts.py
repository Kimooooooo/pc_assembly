...
4. **⛔️ 필수 호환성 규칙 (최우선)**
   - **CPU와 메인보드의 소켓(Socket)**은 반드시 일치해야 합니다. (예: LGA 1700 CPU는 LGA 1700 메인보드)
   - **CPU가 지원하는 RAM 규격**과 **메인보드/RAM 규격(DDR4 또는 DDR5)**은 반드시 일치해야 합니다.
5. **⛔️ 부품 신뢰성 규칙**
   - **단종되었거나 5년 이상된 구형 부품(예: 인텔 제온, 구형 DDR4 저클럭 등)**은 **최대한 추천하지않으며 **, 100만원 이상의 예산에서는 반드시 최신 세대의 현역 부품을 사용할 것.
...
3. **⛔️ 각 조합은 최소 2개 이상의 핵심 부품(CPU, GPU, 메인보드, RAM)이 서로 달라야 함.**
4. **⛔️ 각 조합의 총 가격은 명확하게 달라야 함.** (1순위, 2순위, 3순위의 가격이 동일해서는 안 됨)   
...
4. **각 조합은 명확하게 차별화되어야 함:**
   - **조합 1 (1순위)**: 예산에 가장 근접하고 목적에 최적인 균형 잡힌 조합.
   - **조합 2 (2순위)**: 가성비(예산보다 5~10만원 저렴)를 중시하며, 성능은 약간 낮지만 효율적인 조합.
   - **조합 3 (3순위)**: 특정 부품(예: GPU 성능)을 조금 더 높여 예산에 딱 맞추거나, 또는 예산 여유분을 남긴 조합.
...
**작업:**
위 정보를 바탕으로 최종 호환성을 판단하세요. **특히 CPU-메인보드 소켓 일치 여부와 RAM 규격 일치 여부  전력관련된부분을  재확인하세요.**
...
**절대 규칙:**
1. **⛔️ {compatible_combos} 섹션에 제공된 부품 조합과 총 가격을 절대로 임의로 변경하거나 복사하여 붙여넣어서는 안 됨
2.순위별로 다른 조합 을 보여줘야함
3.각 순위별로 특장점이 구별이되게 보여줘야함 
예시: 
1순위 ... (가장 요구사항에 부합함)
2순위 ... (요구사항에 부합하면서 1순위에 비해 성능이 조금 부족한대신 가격면에서 이득)
3순위 ... (1,2순위에 비해 요구사항에는 턱걸이지만 다른 부분도 보완함)
